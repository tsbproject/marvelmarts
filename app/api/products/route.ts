...
export async function PUT(request: NextRequest) {
  try {
    ...
    const isAdmin = session.user.role === "ADMIN" || session.user.role === "SUPER_ADMIN";
    if (existingProduct.vendorId !== session.user.id && !isAdmin) return NextResponse.json({ success: false, message: "Forbidden" }, { status: 403 });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (product.vendorId !== session.user.id && session.user.role !== "ADMIN" && session.user.role !== "SUPER_ADMIN") {
