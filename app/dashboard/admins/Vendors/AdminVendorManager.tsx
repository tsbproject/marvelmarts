...
interface VendorProfile {
  id: string;
  userId: string; // Required for enforcement actions
  storeName: string;
  status: "PENDING" | "APPROVED" | "REJECTED";
  isSuspended: boolean;
  logoUrl?: string;
...
export default function AdminVendorManager() {
  ...
  // Standard Actions (Approve/Reject)
  async function handleVendorAction(vendorProfileId: string, action: "APPROVE" | "REJECT" | "DELETE") {
    if (action === "DELETE" && !confirm("Permanently delete this vendor?")) return;
    setActionId(`${vendorProfileId}-${action}`);
    try {
      const res = await fetch("/api/admins/vendors/status", {
      ...
      if (res.ok) {
        dispatch(updateVendorStatusInStore({ vendorId: vendorProfileId, action }));
        notifySuccess(`Vendor ${action.toLowerCase()} successful`);
        if (selectedVendor?.id === vendorProfileId) setSelectedVendor(null);
      } else {
        notifyError("Action failed");
  ...
  // Enforcement Actions (Suspend/Restore with Reason)
  async function handleEnforcement() {
    if (!enforcementReason) return notifyError("Reason Required", "Please state why this action is being taken.");
    ...
    try {
      ...
      if (res.ok) {
        dispatch(updateVendorStatusInStore({ vendorId: vendor.id, action }));
        notifySuccess(`Vendor ${vendor.storeName} ${action === "SUSPEND" ? "Suspended" : "Restored"}`);
      ...
      notifyError("Enforcement Failed", "Could not update merchant status.");
    } finally {
      setActionId(null);
    }
  }
                ...
                {selectedVendor.verificationDoc ? (
                  <div className="rounded-[1.5rem] overflow-hidden border-4 border-gray-50 shadow-inner bg-gray-50 group relative">
                    <Image 
                      ...
                      className="w-full h-auto object-contain min-h-[200px]" 
                    />
                    <a href={selectedVendor.verificationDoc} target="_blank" rel="noreferrer" className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity text-white font-bold text-xs">View Full Document</a>
                  </div>
                ) : (
                  <div className="h-48 rounded-[1.5rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 italic text-xs text-center p-4">
                    <ShieldAlert size={32} className="mb-2 opacity-20 text-red-500" />
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {['instagram', 'whatsapp', 'twitter'].map((platform) => {
                      ...
                      return (
                        <div key={platform} className={`flex items-center gap-2 p-2 rounded-xl border ${handle ? 'bg-white border-orange-100' : 'bg-transparent border-gray-200 opacity-40'}`}>
                          <span className="text-[8px] font-black uppercase text-gray-400 w-6">{platform.slice(0, 2)}</span>
                          <p className="text-[10px] font-bold truncate text-[#002B5B]">{handle || "Not set"}</p>
                        </div>
                      );
                    })}
                  </div>
