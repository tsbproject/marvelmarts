...
import confetti from "canvas-confetti";
import DashboardHeader from "@/app/_components/DashboardHeader";
import { Camera, Store, CheckCircle2, Loader2, CreditCard, Globe, Zap, ShieldCheck, ArrowUpRight } from "lucide-react";
import { useNotification } from "@/app/_context/NotificationContext";
...
export default function VendorProfilePage() {
  ...
  // Get current profile data from Redux
  const vendorProfile = useSelector((state: RootState) => state.vendor.profile);
  const boostData = vendorProfile?.boost; // Accessing the boost relationship from the profile
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50/30">
      ...
      {/* PROGRESS & BOOST SUMMARY SECTION */}
      <div className="px-4 md:px-8 mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 w-full">
            ...
            <p className="text-[10px] text-gray-400 font-bold mt-2 uppercase tracking-tighter">
              {progress < 100 ? "Complete your profile to build buyer trust" : "Your profile is optimized for sales!"}
            </p>
          </div>
        </div>

        {/* CURRENT CREDITS WIDGET */}
        <div className="bg-[#002B5B] p-6 rounded-[2.5rem] shadow-xl shadow-blue-900/20 text-white flex items-center justify-between relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-widest text-white/50 mb-1">Available Credits</p>
            <h4 className="text-4xl font-black italic tracking-tighter">
              {boostData?.credits || 0}
            </h4>
          </div>
          <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center text-[#F7931E] group-hover:scale-110 transition-transform relative z-10">
      ...
      <div className="p-4 md:p-8 space-y-10">
        ...
        <div className="bg-white rounded-4xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-gray-100 flex justify-between items-center">
            <h3 className="text-2xl font-extrabold text-[#002B5B] uppercase italic tracking-tighter">0. Purchase Credits</h3>
            <div className="flex items-center gap-2 px-4 py-2 bg-green-50 rounded-full">
               <ShieldCheck size={14} className="text-green-600" />
               <span className="text-[10px] font-black text-green-700 uppercase tracking-widest">Secure Payments</span>
            </div>
          </div>
          <div className="p-2">
            {vendorProfile?.id && (
        ...
        <div className="flex justify-end pb-20">
          <button 
            ...
            className={`px-16 py-6 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.2em] transition-all shadow-2xl flex items-center gap-4 ${
              progress === 100 
              ? "bg-green-600 text-white shadow-green-600/30 hover:scale-105 active:scale-95" 
              : "bg-[#002B5B] text-white shadow-blue-900/30 hover:scale-105 active:scale-95"
            } disabled:opacity-50 disabled:scale-100`}
          >
