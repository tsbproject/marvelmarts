...
export default function VendorChatThread() {
  ...
  useEffect(() => {
    ...
    const fetchThread = async () => {
      try {
        ...
        if (!res.ok) throw new Error(data.error || "Failed to sync frequency");
    ...
    channel.bind("messages-read", ({ readerId }: { readerId: string }) => {
      if (readerId !== session.user.id) {
        setMessages((prev) =>
          prev.map((m) =>
            m.senderId === session.user.id ? { ...m, isRead: true } : m
          )
        );
      }
    });
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (!input.trim() || conversation?.status === "CLOSED") return;
  ...
  const handleEndChat = async () => {
    if (!id || conversation?.status === "CLOSED" || endingChat) return;

    try {
      setEndingChat(true);

      const response = await fetch(`/api/vendors/conversations/${id}/end`, {
        method: "POST",
      });
      ...
      setConversation((prev) =>
        prev
          ? {
              ...prev,
              status: "CLOSED",
              endedAt: data?.conversation?.endedAt || new Date().toISOString(),
              endedByRole: data?.conversation?.endedByRole || "VENDOR",
            }
          : prev
      );
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center gap-3 min-h-[50vh] justify-center">
        <div className="w-2 h-2 bg-[#F7931E] rounded-full animate-bounce" />
        <div className="text-[10px] font-black uppercase tracking-widest text-[#002B5B]">
          Syncing Frequency...
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-[85vh] bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
      <div className="p-4 border-b border-gray-50 flex items-center justify-between bg-white sticky top-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {conversation?.visitorName && (
              <p className="text-[9px] text-[#F7931E] font-bold uppercase tracking-widest mt-1">
                Guest: {conversation.visitorName}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={chatClosed || endingChat}
            className="inline-flex items-center gap-2 px-4 py-3 rounded-2xl bg-[#002B5B] text-white text-[9px] font-black uppercase tracking-widest hover:bg-[#001f45] transition-all disabled:opacity-50"
          >
            <PhoneOff size={14} />
            {chatClosed ? "Chat Ended" : endingChat ? "Ending..." : "End Chat"}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-[#FBFBFB]/50">
        {chatClosed && (
          <div className="p-3 rounded-2xl border border-red-100 bg-red-50">
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] p-4 rounded-2xl text-sm shadow-sm relative ${
                    isMe
                      ? "bg-[#002B5B] text-white rounded-tr-none"
                      : "bg-white border border-gray-100 text-gray-800 rounded-tl-none"
                  }`}
                >
                  {msg.productId && (
                    ...
                    >
                      ...
                      <div className="flex flex-col overflow-hidden">
                        <span className="text-[7px] font-black uppercase text-[#F7931E]">
                          Inquiry Asset
                        </span>
                        <span
                          className={`text-[10px] font-bold truncate ${
                            isMe ? "text-white" : "text-[#002B5B]"
                          }`}
                        >
                          {msg.productPrice}
                        </span>
                  ...
                  <p className="leading-relaxed font-medium pb-2 whitespace-pre-wrap">
                    {msg.content}
                  </p>
      ...
      <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-100 flex gap-2">
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder={chatClosed ? "This chat has ended" : "Type your tactical response..."}
          disabled={chatClosed}
          className="flex-1 bg-gray-50 border-none rounded-2xl px-5 py-3 text-sm focus:ring-2 focus:ring-[#F7931E]/20 outline-none transition-all disabled:cursor-not-allowed disabled:opacity-70"
        />
        <button
          type="submit"
          disabled={!input.trim() || chatClosed}
          className="bg-[#F7931E] disabled:opacity-50 text-white p-3 rounded-2xl hover:bg-[#E08214] transition-all shadow-lg shadow-[#F7931E]/20"
        >
          <Send size={18} />
        </button>
