...
//   // FIXED NAVIGATION: Using a single handler for the whole card
...
export default function ProductCard({ 
  ...
  // FIXED NAVIGATION: Logic updated to ensure we don't land on homepage
  const handleCardClick = (e: React.MouseEvent) => {
    const target = e.target as HTMLElement;
    if (target.closest('button')) return;
    ...
    if (onViewDetails) {
      ...
      // Logic: Ensure we get a string. If .slug is an object, take .current.
      const slugValue = typeof product.slug === 'object' ? (product.slug as any).current : product.slug;
      const identifier = slugValue || product.id || (product as any)._id;

      if (identifier) {
        router.push(`/products/${identifier}`);
      } else {
        console.error("Critical: No slug or ID found for", product.title);
      }
    }
  };
