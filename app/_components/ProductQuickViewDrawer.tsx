...
//                     className="w-full bg-accent-navy text-gray-50 py-3.5 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 border border-gray-100 active:bg-black transition-all z-[110]"
//                   >
//                     View Full Details
//                     <ArrowRight size={14} />
//                   </a>
...
import { X, ShoppingCart, Star, ArrowRight } from "lucide-react";
import Image from "next/image";
import Link from "next/link"; // Reverting to Link but with absolute positioning fix
import { formatNaira } from "@/app/lib/FormatNaira";
import { SerializedProduct } from "@/types/product";
import { useDispatch } from "react-redux";
import { addToCart } from "@/store/cartSlice";
...
export default function ProductQuickViewDrawer({ isOpen, onClose, product }: DrawerProps) {
  ...
  // Crucial: Determine the correct path. Fallback to ID if slug is missing.
            ...
            <div className="flex flex-col gap-4">
              <div className="relative w-full h-48 bg-brand-ghost rounded-2xl overflow-hidden">
                <Image
                  ...
                  fill
                  className="object-contain p-4"
                  priority // Ensures image loads fast on mobile
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-baseline gap-3">
                  ...
                  {product.discountPrice && (
                    <span className="text-sm text-brand-gray line-through font-bold italic opacity-50">
                      {formatNaira(product.price)}
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2 pt-2">
                  ...
                  <button
                    onClick={handleAddToCart}
                    className="w-full bg-brand-navy text-brand-white py-3.5 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
                  >
                    <ShoppingCart size={16} />
                    Add to Cart
                  </button>

                  {/* View Details - Using Link with prefetch for speed */}
                  <Link
                    href={productPath}
                    onClick={() => onClose()}
                    className="w-full bg-brand-orange text-brand-white py-3.5 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 active:scale-95 transition-all touch-manipulation"
                  >
                    View Full Details
                    <ArrowRight size={14} />
                  </Link>
