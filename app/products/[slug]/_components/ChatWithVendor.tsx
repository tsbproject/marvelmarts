...
export default function ChatWithVendor({
  ...
  const [isOpen, setIsOpen] = useState(false);
  const [activeConversationId] = useState<string>("");
  ...
  return (
    <>
      <button
        onClick={handleStartChat}
        className="w-full flex items-center justify-center gap-2 py-4 bg-[#002B5B] text-white rounded-2xl font-black uppercase text-sm tracking-[0.2em] hover:bg-[#F7931E] transition-all group"
      >
        <MessageSquare size={18} className="group-hover:scale-110 transition-transform" />
        Chat with Seller
      </button>
