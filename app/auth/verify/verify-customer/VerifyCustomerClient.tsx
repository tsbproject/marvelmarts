...
//             console.log("VERIFIED LOGIN RESULT:", loginRes);
...
//             // Wait for session to actually become available
//             let sessionReady = false;

//             for (let i = 0; i < 10; i++) {
//               const freshSession = await getSession();
//               console.log("POST-VERIFY SESSION CHECK:", freshSession);
...
//             if (!sessionReady) {
//               notifyError("Auto login did not complete in time. Please sign in manually.");
//               router.push(`/auth/sign-in?redirect=${encodeURIComponent(redirectTarget)}`);
//               return;
//             }

//             // Hard navigation so checkout reads the fresh session cookie
...
export default function VerifyCustomerClient() {
  ...
  async function waitForSession(maxAttempts = 12, delayMs = 300) {
    for (let i = 0; i < maxAttempts; i++) {
      const session = await getSession();
      console.log("POST-VERIFY SESSION CHECK:", session);
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (!data?.autoLoginToken) {
        notifyError("Verification succeeded, but auto-login token was not returned.");
        window.location.href = `/auth/sign-in?redirect=${encodeURIComponent(redirectTarget)}`;
        return;
      }
