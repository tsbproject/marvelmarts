...
// export type VerificationStatus = "NOT_STARTED" | "PENDING" | "APPROVED" | "REJECTED" | "PENDING_REVIEW";
...
//     try {
//       // 1. Send the unified data to the backend
//       // This now includes: slug, storeName, bio/description, logoUrl, coverUrl, 
//       // social handles, bank details, and the completion flags (storeDone, payoutsDone).
//       const response = await axios.patch("/api/vendors/profile/settings", settingsData);
      
//       const updatedProfile = response.data.profile;

//       // 2. Update Redux State
//       // We ensure the onboarding flags are synced so the Dashboard banner 
//       // disappears immediately when storeDone and productDone are true.
...
//       //     storeDone: updatedProfile.storeDone, // This flag now includes 'slug' completion
//       //     payoutsDone: updatedProfile.payoutsDone, // Track payment setup status
...
//     } catch (error: any) {
//       // Extract specific error (e.g., "Slug already taken") to show in Notification
//       const errorMessage = error.response?.data?.message || "Failed to update store settings";
...
//       // Also sync roles if they exist in the profile payload
...
//         onboarding: Onboarding; 
//         balance: number; 
//         roles?: UserRole[]; // FIXED: Added roles to payload
//         lastSyncedAt?: string 
//       }>
//     ) => {
...
const vendorSlice = createSlice({
  ...
  reducers: {
    ...
    updateVendorStatusInStore: (
      ...
      if (action.payload.status !== undefined) {
        vendor.status = action.payload.status as VerificationStatus;
      }
